...
def fmt_col_name(label: str) -> str:
    ...
    # Step 2: Split by spaces and underscores, then rejoin with a single underscore
    words = label.replace('_', ' ').split()
    formatted_label = '_'.join(words)

    # Step 3: Convert to lowercase
    return formatted_label.lower()

# Example usage
print(fmt_col_name(' Close'))  # Output: 'close'
print(fmt_col_name('Adj    Close'))  # Output: 'adj_close'
print(fmt_col_name('Adj_Close '))  # Output: 'adj_close'
print(fmt_col_name('  adj    Close  '))  # Output: 'adj_close'
...
def fmt_ticker(value: str) -> str:
    ...
    # Step 1: Strip leading/trailing spaces and quotes
    value = value.strip().strip("'\"")

    # Step 2: Convert to uppercase or lowercase based on context
    if as_column_label:
        return value.lower()
    else:
        return value.upper()
# Example usage
print(fmt_ticker(" AAPL "))  # Output: 'AAPL'
print(fmt_ticker(" 'msft' "))  # Output: 'MSFT'
print(fmt_ticker(" GOOG ", as_column_label=True))  # Output: 'goog'
